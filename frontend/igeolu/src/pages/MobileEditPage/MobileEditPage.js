...
function MobileEditPage() {
    ...
    const [monthlyRent, setMonthlyRent] = useState(propertyData?.monthlyRent || '');
    const [area, setArea] = useState(propertyData?.area || '');
    const [approvalDate, setApprovalDate] = useState(propertyData?.approvalDate || '');
    const [currentFloor, setCurrentFloor] = useState(propertyData?.currentFloor || '');
    const [totalFloors, setTotalFloors] = useState(propertyData?.totalFloors || '');
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            // 데이터 유효성 검사 추가
            if (!description || !deposit || !monthlyRent || !area || !approvalDate || 
                !currentFloor || !totalFloors || !address) {
            ...
            const response = await axios.put(`/api/properties/${propertyData.propertyId}`, formData, {
                headers: {
    ...
    const handleDelete = async () => {
        if (window.confirm('정말로 이 매물을 삭제하시겠습니까?')) {
            try {
                ...
                const response = await axios.delete(`/api/properties/${propertyData.propertyId}`, {
                    headers: {
                        'Authorization': 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJ1c2VySWQiOjMyLCJyb2xlIjoiUk9MRV9SRUFMVE9SIiwiaWF0IjoxNzM4OTAyOTM4LCJleHAiOjE3NDAxMTI1Mzh9.nE5i5y2LWQR8Cws172k0Ti15LumNkDd0uihFYHQdnUg'
                    }
                });
                ...
                alert(`매물 삭제에 실패했습니다. ${error.response?.data?.message || ''}`);
            }
        }
    };
        ...
        <div className="mobile-edit-page-container">
            <div className="mobile-edit-page">
                ...
                <div className="mobile-edit-page__introduce">
                    <label>매물 소개</label>
                    <input placeholder="매물 소개를 입력해 주세요." value={description} onChange={(e) => setDescription(e.target.value)}></input>
                </div>
                ...
                <div className="mobile-edit-page__deposit-rent">
                    ...
                    <div className="deposit-input">
                        <input placeholder="보증금" value={deposit} onChange={(e) => setDeposit(e.target.value)}></input>
                        <input placeholder="월세" value={monthlyRent} onChange={(e) => setMonthlyRent(e.target.value)}></input>
                    </div>
                </div>
                <div className="mobile-edit-page__area">
                    <label>면적</label>
                    <input placeholder="00m²" value={area} onChange={(e) => setArea(e.target.value)}></input>
                </div>
                ...
                <div className="mobile-edit-page__floor">
                    ...
                    <div className="floor-input">
                        <input placeholder="해당층" value={currentFloor} onChange={(e) => setCurrentFloor(e.target.value)}></input>
                        <input placeholder="총층" value={totalFloors} onChange={(e) => setTotalFloors(e.target.value)}></input>
                    </div>
                </div>
