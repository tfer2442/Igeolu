...
function MapPage() {
  ...
  // 이 부분 네비게이션 바 작동 때문에 바꾼다. 아마 원래 기능에는 영향 없을 듯 - from.오승우 -
  ...
  // 이거 추가했어. URL바뀔때마다 '원룸'/'공인중개사'버튼 업데이트 하는 거임. 마찬가지로 네비게이션 바 관련 - from. 오승우 -
  // URL의 type 파라미터가 변경될 때마다 activeMenu 업데이트
  useEffect(() => {
    if (typeParam) {
      setActiveMenu(typeParam);
      // 메뉴 변경 시 필요한 데이터 fetch
      if (typeParam === 'agent') {
        fetchRealtors();
      } else {
        fetchSearchResults();
