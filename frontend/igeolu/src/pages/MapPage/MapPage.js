...
function MapPage() {
    ...
    const fetchRealtors = async () => {
        try {
            ...
            if (!selectedCity || !selectedDistrict || !selectedNeighborhood) {
                ...
                setSearchResults(realtors);
                setPropertyMarkers(realtors); // 전체 공인중개사 마커 표시
                return;
            }
            ...
            if (selectedCity && selectedDistrict && selectedNeighborhood) {
                try {
                    ...
                    if (dongCode) {
                        ...
                        setSearchResults(realtors);
                        setPropertyMarkers(realtors); // 해당 지역 공인중개사 마커 표시
                    }
                } catch (error) {
                    console.error('Error fetching coordinates or realtors:', error);
                    setSearchResults([]);
    ...
    // 초기 로딩시 type=agent 처리를 위한 useEffect
    useEffect(() => {
        console.log('MapPage - Initial type parameter check:', typeParam);
        const initializeByType = async () => {
            if (typeParam === 'agent') {
                try {
                    console.log('MapPage - Fetching initial realtors data');
                    const response = await axios.get(`${API_BASE_URL}/api/users/realtors`);
                    console.log('MapPage - Fetched initial realtors:', response.data.length);
                    ...
                    console.log('MapPage - Valid initial realtors with coordinates:', validRealtors.length);
                    ...
                    console.error('MapPage - Error fetching initial realtors:', error);
                    setSearchResults([]);
                    setPropertyMarkers([]);
                }
            }
                        ...
                        <div className='map-container'>
                            <div className='map-content'>
                                ...
                                >
                                    ...
                                    {activeMenu === 'room' && !selectedItem && propertyMarkers.length === 0 && searchResults.map((item, index) => {
                                        if (!item?.latitude || !item?.longitude) return null;
