...
public class OpenViduService {
	...
	public boolean isModeratorSessionValid(String sessionId, String token) {
		try {
			...
			if (token.isEmpty())
				return false;
			if (!this.moderatorsCookieMap.containsKey(sessionId))
				return false;
	...
	public boolean isParticipantSessionValid(String sessionId, String cookie) {
		...
		try {
			if (!this.participantsCookieMap.containsKey(sessionId))
				return false;
			if (cookie.isEmpty())
				return false;
			...
			for (String token : storedTokens) {
				...
				if (sessionId.equals(cookieSessionId) && cookieToken.equals(storedToken) && cookieDate.equals(
					storedDate)) {
					return true;
				}
			}
	...
	public List<Recording> listRecordingsBySessionIdAndDate(String sessionId, long date)
		...
		for (Recording recording : recordings) {
			if (recording.getSessionId().equals(sessionId)
				&& recording.getCreatedAt() + recording.getDuration() * 1000 >= date) {
